...
class Doper:
    ...
    def get_dopants(self) -> dict:
        ...
        higher_charge = {
            "element": utilities.parse_spec(cat)[0],
            "charge": utilities.parse_spec(cat)[1],
        }
        lower_charge = {
            "element": utilities.parse_spec(an)[0],
        ...
        parsed_poss_n_type_cat = [
            utilities.unparse_spec(specie) for specie in poss_n_type_cat
        ]
        parsed_poss_p_type_cat = [
            utilities.unparse_spec(specie) for specie in poss_p_type_cat
        ]
        # anion loop to calculate substitution probability
        parsed_poss_n_type_an = [
            utilities.unparse_spec(specie) for specie in poss_n_type_an
        ]
        parsed_poss_p_type_an = [
            utilities.unparse_spec(specie) for specie in poss_p_type_an
        ]

        # calculate substitutional probability for all elememts stored in the list
        n_type_cat = [
            (specie, CM.sub_prob(cat, specie)) for specie in parsed_poss_n_type_cat
        ]
        p_type_cat = [
            (specie, CM.sub_prob(cat, specie)) for specie in parsed_poss_p_type_cat
        ]
        n_type_an = [
            (specie, CM.sub_prob(an, specie)) for specie in parsed_poss_n_type_an
        ]
        p_type_an = [
            (specie, CM.sub_prob(an, specie)) for specie in parsed_poss_p_type_an
        ]
        ...
        self.results = {
            "n-type cation substitutions": n_type_cat[: self.num_dopants],
