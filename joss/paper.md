...
SMACT v4 meets these needs by providing a unified Python library where a researcher can (i) enumerate and filter compositions, (ii) predict likely crystal structures from data-mined substitution probabilities, (iii) estimate material properties directly from stoichiometry without requiring structural information, and (iv) identify candidate dopants for known host materials. The stages share a common representation of compositions and oxidation-state-resolved species, so screening output can be passed on to structure, property or dopant prediction as the question requires, and of the four only structure prediction requires structural input, in the form of the parent structures it substitutes into; a campaign can therefore narrow a chemical space before incurring any structural cost. The library is designed for desktop-scale computation, with data caching and multiprocessing support that enables screening of millions of compositions in minutes.
...
SMACT occupies a distinct niche: it works at the composition level, upstream of structure-based tools, to define which regions of chemical space merit further investigation. This top-down, composition-first approach complements structure-based workflows with lower-cost tools that scale to vast chemical spaces. The structure prediction and property prediction modules added since v2.0 provide natural bridges between SMACT's compositional screening and downstream structural analysis tools. The `ElementEmbeddings` integration [@Antunes2022] further connects SMACT to modern representation learning approaches for materials informatics [@Onwuli2024].

The distinction is sharpest where the functionality appears to overlap. `Pymatgen` implements the same data-mined ionic substitution model [@Hautier2011] in its `SubstitutionProbability`, `SubstitutionPredictor`, `Substitutor` and `SubstitutionPredictorTransformation` classes, and predicts dopants through `get_dopants_from_substitution_probabilities` and `get_dopants_from_shannon_radii`. Both of those dopant predictors require a crystal structure decorated with oxidation states, and the radius-based one additionally requires a bonded structure graph, because Shannon radii are defined per coordination number. SMACT's `Doper` takes a list of species instead, and so answers what could be doped into a _composition_ before any structure is known, which is the regime in which the candidate space is largest. The same reasoning shapes SMACT's structure prediction, which pairs the substitution model with a local, species-indexed database of known structures so that candidate compositions are matched against parent structures in bulk rather than one transformation at a time.

SMACT therefore extends the substitution model rather than reimplementing it: substitution tables can be replaced by learned, oxidation-state-aware species embeddings [@Onwuli2024], and dopant candidates on cation sites are ranked by selectivity between competing host sites as well as by raw probability. Both presuppose the structure-free setting, which is why they sit here rather than upstream in `pymatgen`, on which SMACT continues to depend for structure handling and I/O.
...
**Dopant prediction.** Enables high-throughput identification of p-type and n-type dopants for a given host composition. Candidates are filtered on accessible oxidation states, an n-type dopant being required to carry a higher charge than the host ion it replaces and a p-type dopant a lower one, in both cases retaining the sign of the site, then scored either by data-mined substitution probability [@Hautier2011] or by cosine similarity in a species embedding [@Onwuli2024]. For substitutions on cation sites, the default ranking combines that score with a selectivity term weighing a dopant's affinity for one host site against its affinity for the others. The workflow takes a composition rather than a structure, so no ionic-radius criterion is applied: Shannon radii are defined per coordination number, which is not known until a structure is.
