...
SMACT therefore extends the substitution model rather than reimplementing it: substitution tables can be replaced by learned, oxidation-state-aware species embeddings [@Onwuli2024], and dopant candidates are ranked by selectivity between competing host sites as well as by raw probability. Both presuppose the structure-free setting, which is why they sit here rather than upstream in `pymatgen`, on which SMACT continues to depend for structure handling and I/O.
