...
def radius_covalent(element):
    with open(path.join(smact.data_directory,
                        'Covalent_radii.csv'), 'rU') as f:
        reader = csv.reader(f)
        reader.next()  # Skip the first row which is titles
        r_covalent = 0
        for row in reader:
            symbols = list(row[0])
            # Some symbols specify a shell e.g. "Cu(d)";
...
def formal_charge(element):
    """Given an elemental symbol, return a list of possible oxidation states"""
    with open(path.join(smact.data_directory,
...
for element in element_set:
    with open(path.join(smact.data_directory, 'shannon_radii.csv'), 'rU') as f:
        ...
        r_shannon = False
        for row in reader:
            if (row[2] == "4_n" and row[0] == element
                and row[0] not in tetra_coord_elements):
...
for a_element in tetra_coord_elements:
    ...
    b_elements = [b for b in tetra_coord_elements
                  if (tetra_coord_elements.index(b)
                      > tetra_coord_elements.index(a_element))]
    if a_formal_charge:  # i.e. a_formal_charge has a value
        for a_ox in a_formal_charge:
            a_shan = radius_shannon(a_element, a_ox)

            if a_shan is False:     # Fall back to covalent radius
                                    # if Shannon unavailable
                for b_element in b_elements:
                    ...
                    if b_formal_charge:  # i.e. b_formal_charge has a value
                        # Check for charge-neutral combinations and write to
                        # array. Note that only the elements are recorded;
                        # multiple charge combinations for one element pair
                        # will be overwritten
                        for b_ox in (x for x in b_formal_charge
                                     if x + a_ox == lattice_charge):
                            ...
                            output.append([a_element, b_element, hhi_rp,
                                           method, a, b, c, inner_space])

            else:    # Use Shannon radius
                for b_element in b_elements:
                    b_formal_charge = formal_charge(b_element)  # check again
                    if b_formal_charge:
                        # Check for charge-neutral combinations
                        # and write to array
                        for b_ox in b_formal_charge:
                            if radius_shannon(b_element, b_ox) is False:
                                # (Charge of lattice)
...
hhi_filtered_output = [compound for compound in output
                       if ((compound[2] <= hhi_max)
                           and (compound[2] >= hhi_min))]


def get_volume_key(compound):
    return compound[7]
size_sorted_output = sorted(hhi_filtered_output, key=get_volume_key, reverse=True)
...
with open('candidates.csv', 'w') as csvfile:
    ...
    csv_writer.writerow(["Element A", "Element B",
                         "HHI index (geometric mean)", "method",
                         "a / AA", "b / AA", "c / AA", "pore volume / AA^3"])
    csv_writer.writerows(size_sorted_output)
