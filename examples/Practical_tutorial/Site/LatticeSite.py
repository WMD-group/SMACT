...
class Pair(object):
	...
	def __init__(self, material1,material2,surface1,surface2,multiplicity1,multiplicity2,strains,csl):
		"""
		Attributes:
			...
			Pair.csl : the site matching, a float
		"""
...
def surface_ratios(surface_a,surface_b,threshold=0.05,limit=5):
	...
	Returns:
		exists : a bool of whether the match was found
...
# Set the  values for material B
	...
	for index_b in indices_b:
			if index_b != (0, 0, 0):
				...
				epitaxy,a,b,strains = surface_ratios(surface_vector_1,surface_vector_2,threshold=options.strain,limit=5)
				if epitaxy:
					if options.verbose:
						print("------   ------   ------   ------   ------")
					...
					new = Pair(material1,material2,index_a,index_b,a,b,strains,0.)
					isnew = True
...
for pair in matched_pairs:
	...
	surface_1 = ("".join(map(str,pair.surface1)))
	surface_2 = ("".join(map(str,pair.surface2)))
	multiplicity1 = pair.multiplicity1
	...
	if len(surfs_1) > 0 and len(surfs_2) > 0:
		pair.csl = SiteMatch.find_max_csl(surfs_1,surfs_2,multiplicity1,multiplicity2)
	print(pair.surface1, pair.surface2, ( pair.csl * 100 ) / ( 1 + np.mean(pair.strains[:2]*100 )))
