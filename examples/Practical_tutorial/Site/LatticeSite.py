            ...
            if epitaxy:
                ...
                for old in matched_pairs:
                    if (
                        ...
                        if (
                            new.surface1 == old.surface1
                            and new.surface2 == old.surface2
                        ):
                            isnew = False
                    else:
                        continue
...
for pair in matched_pairs:
    ...
    if len(surfs_1) > 0 and len(surfs_2) > 0:
        pair.csl = SiteMatch.find_max_csl(
            surfs_1, surfs_2, multiplicity1, multiplicity2
        )
    print(
        pair.surface1,
        pair.surface2,
        (pair.csl * 100) / (1 + np.mean(pair.strains[:2] * 100)),
    )
